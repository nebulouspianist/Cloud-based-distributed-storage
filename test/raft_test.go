...
func TestRaftSetLeader(t *testing.T) {
	...
	// t.Fatalf("set leader test successfully passed")
}
...
func TestRaftFollowersGetUpdates(t *testing.T) {
	...
	// TEST
	leaderIdx := 0
	success, err := test.Clients[leaderIdx].SetLeader(test.Context, &emptypb.Empty{})
	...
	success, err = test.Clients[leaderIdx].SendHeartbeat(test.Context, &emptypb.Empty{})

	if err != nil {
		log.Println(err.Error())
	}

	fmt.Println("line 108", success.Flag)
